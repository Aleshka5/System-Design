...
def send_message(login: str, target_login: str, body: str, current_user: str = Depends(get_current_client)):
    if login in client_db.keys() and target_login in client_db.keys():
        ...
        message = Message(body=body, user_name=user.name if getattr(
            user, "name") else login)
...
def get_chat(login: str, target_login: str, current_user: str = Depends(get_current_client)):
...
def create_post(login: str, post_txt: str, current_user: str = Depends(get_current_client)):
    if login in client_db.keys():
        user = client_db[login]
...
def get_user_by_name(name: str, surname: str, current_user: str = Depends(get_current_client)):
